...
FilePath FilePath::InsertBeforeExtensionASCII(StringPiece suffix) const {
  std::string suffixStr(suffix.data(), suffix.length());
  DCHECK(IsStringASCII(suffix));
#if defined(OS_WIN)
  return InsertBeforeExtension(ASCIIToWide(suffixStr));
#elif defined(OS_POSIX)
  return InsertBeforeExtension(suffixStr);
#endif
}
...
FilePath FilePath::AppendASCII(StringPiece component) const {
  std::string componentStr(component.data(), component.length());
  DCHECK(base::IsStringASCII(componentStr));
#if defined(OS_WIN)
  return Append(ASCIIToWide(componentStr));
#elif defined(OS_POSIX)
...
FilePath FilePath::NormalizePathSeparatorsTo(CharType separator) const {
  ...
  DCHECK(kSeparators + kSeparatorsLength != std::find(kSeparators, kSeparators + kSeparatorsLength, separator));
