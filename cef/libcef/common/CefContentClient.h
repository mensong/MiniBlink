...
public:
    ...
    CefRefPtr<CefApp> rendererApplication() const { return m_rendererApplication; }
    CefRefPtr<CefApp> SetRendererApplication(CefRefPtr<CefApp> rendererApplication) { 
        return m_rendererApplication = rendererApplication;
    }
...
private:
    ...
    CefRefPtr<CefApp> m_rendererApplication; // 渲染进程会调用CefExecuteProcess再传一次application
