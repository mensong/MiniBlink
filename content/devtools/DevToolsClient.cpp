...
void DevToolsClient::sendMessageToBackend(const blink::WebString& message)
{
	...
	//Gergul: 防止调出DevTools窗口后快速关闭主窗口造成的崩溃
    if (NULL != m_devToolsAgent && !m_devToolsProtocolDispatcher->dispatcher(messageStr.utf8().data()))
...
{
	//Gergul: 防止重复的defersLoading
	m_defersLoadingframes.clear();
...
{
	int n = m_defersLoadingframes.size();
