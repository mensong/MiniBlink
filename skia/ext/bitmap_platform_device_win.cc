...
SkBaseDevice* BitmapPlatformDevice::onCreateDevice(const CreateInfo& createInfo, const SkPaint*) {
  //SkASSERT(createInfo.fInfo.colorType() == kPMColor_SkColorType);
  SkASSERT(createInfo.fInfo.colorType() == kN32_SkColorType);
  return BitmapPlatformDevice::CreateAndClear(createInfo.fInfo.width(), createInfo.fInfo.height(),
      createInfo.fInfo.isOpaque());
}
