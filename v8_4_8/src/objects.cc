...
DWORD isCreatingDataPropertyNow = 0;// 在属性访问器里调用CreateDataProperty会重入
  ...
  for (; it->IsFound(); it->Next()) {
    switch (it->state()) {
      ...
      case LookupIterator::ACCESSOR: {
        ...
        // data property.
		bool bCreatingDataProperty = false; ;// TlsAlloc(void);
		if (0 != isCreatingDataPropertyNow)
			bCreatingDataProperty = (1 == (int)TlsGetValue(isCreatingDataPropertyNow));

		if (accessors->IsExecutableAccessorInfo() && handling == DONT_FORCE_FIELD && !bCreatingDataProperty) {
...
Maybe<bool> JSObject::CreateDataProperty(LookupIterator* it,
                                         Handle<Object> value) {
  if (0 == isCreatingDataPropertyNow)
  	isCreatingDataPropertyNow = TlsAlloc();
  TlsSetValue(isCreatingDataPropertyNow, (LPVOID)1);
  ...
  TlsSetValue(isCreatingDataPropertyNow, (LPVOID)0);
  return Just(true);
}
