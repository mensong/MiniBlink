...
private:
    ...
    void onNewWindowInBlinkThread(int width, int height, const CreateWindowParam* createWindowParam)
    {
        ...
        const wchar_t* htmlW = L"<html><head></head><body style='font-size:16px;'>"
            L"输入网址: &nbsp<input id=myurl style='width:400px;' onkeydown=if(event.keyCode==13)GoURL(); value='https://www.bilibili.com/video/av16935770/?spm_id_from=333.334.home_popularize.4'><button onclick=GoURL();>GO</button><hr />"
